...
SGL_PY_EXPORT(core_input)
{
    ...
    nb::class_<KeyboardEvent>(m, "KeyboardEvent", D(KeyboardEvent))
        .def(nb::init<>())
        .def_rw("type", &KeyboardEvent::type, D(KeyboardEvent, type))
        .def_rw("key", &KeyboardEvent::key, D(KeyboardEvent, key))
        .def_rw("mods", &KeyboardEvent::mods, D(KeyboardEvent, mods))
        .def_rw("codepoint", &KeyboardEvent::codepoint, D(KeyboardEvent, codepoint))
    ...
    nb::class_<MouseEvent>(m, "MouseEvent", D(MouseEvent))
        .def(nb::init<>())
        .def_rw("type", &MouseEvent::type, D(MouseEvent, type))
        .def_rw("pos", &MouseEvent::pos, D(MouseEvent, pos))
        .def_rw("scroll", &MouseEvent::scroll, D(MouseEvent, scroll))
        .def_rw("button", &MouseEvent::button, D(MouseEvent, button))
        .def_rw("mods", &MouseEvent::mods, D(MouseEvent, mods))
    ...
    nb::class_<GamepadEvent>(m, "GamepadEvent", D(GamepadEvent))
        .def(nb::init<>())
        .def_rw("type", &GamepadEvent::type, D(GamepadEvent, type))
        .def_rw("button", &GamepadEvent::button, D(GamepadEvent, button))
    ...
    nb::class_<GamepadState>(m, "GamepadState", D(GamepadState))
        .def(nb::init<>())
        .def_rw("left_x", &GamepadState::left_x, D(GamepadState, left_x))
        .def_rw("left_y", &GamepadState::left_y, D(GamepadState, left_y))
        .def_rw("right_x", &GamepadState::right_x, D(GamepadState, right_x))
        .def_rw("right_y", &GamepadState::right_y, D(GamepadState, right_y))
        .def_rw("left_trigger", &GamepadState::left_trigger, D(GamepadState, left_trigger))
        .def_rw("right_trigger", &GamepadState::right_trigger, D(GamepadState, right_trigger))
        .def_rw("buttons", &GamepadState::buttons, D(GamepadState, buttons))
